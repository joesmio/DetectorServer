...
//printf("]]] lockok %d:%s\n", __LINE__, __FILE__);fflush(stdout);
    ...
    for (size_t index = 0; index < current_listeners.size(); ++index)
    {
		...
		else
		{
				...
				if (count) {
					...
					for (int index = 0; index < count; ++index)
					{
						...
						if (chan[index] == 2 && startflag_g2 == 1) {
							//stop
							t = time[index] - start_g2; 
							//if (t < 6400) // 64 * 800 ns else throw away (try not throwing away)
							listener->histogram_counts[0][t%6400]++; // get t in terms of 0.5 ns i.e. /32
							
																		
							//listener->histogram_counts[0][t %1600]++;
			...
			else if (listener->user_platform.compare("odmr") == 0)
			{
				...
				if (count)
				{
					...
					for (int index = 0; index < count; index++)
					{
						...
						*/
						...
						if (odmr_startflag == 1 && chan[index] == 13) {
						...
						if (odmr_startflag == 2 && chan[index] == 14) {
							...
							if (duration > 6400 * 1000) {
								...
								if (counter < px_resolution_y) {
									...
									//else
									listener->histogram_counts[counter][px] = (int)((6400000000 / duration)*countme);
									
									
									printf("PONG ");
									printf("@ %i ", counter);
									printf("PONG %i ", px);
									printf("counts %i ", listener->histogram_counts[counter][px]);
									printf(" duration %i \n", duration/ 6400);
								}

								// New line if at end of line in either direction
								if (px == histogram_channels - 1) {

									counter++;
									px = 0;

									//printf("] \n");
									printf("On line %i, ", counter);
									printf("sent %i \n", sentlines);
									//printf("[");
						...
						if (((chan[index] == 1) || (chan[index] == 2)) && (odmr_startflag == 2)) {
							countme++;
						}
					}
				}
				...
				if ((startflag == 0)  || (listener->histogram_counts.size() == 0)){
					...
					if (listener->user_platform.compare("3dscanner") == 0) {
						...
						if (histogram_channels < 400 && histogram_channels > 5) {
							px_resolution_x = histogram_channels;
							px_resolution_y = z_res*histogram_channels;
					...
					if (listener->user_platform.compare("stabilise") == 0) { // tracking
						...
						if (histogram_channels < 400 && histogram_channels > 1) {
							px_resolution_x = histogram_channels;
							px_resolution_y = 3; // x, y, z  ? * number of tries i.e. 3 * 65
				...
				if (count)
				{
					...
					for (int index = 0; index < count; index++)
					{
						...
						*/
						...
						if (((chan[index] == 1) || (chan[index] ==2)) && (startflag == 2)) {
							countme++;
						}
					}
				}
	...
	bool scannerconnected = 0;
		for (size_t index = 0; index < current_listeners.size(); ++index)
        {
			...
			if (listener->user_platform.compare("scanner") == 0 || listener->user_platform.compare("stabilise") == 0 || listener->user_platform.compare("3dscanner") == 0 || listener->user_platform.compare("odmr")==0)
				scannerconnected = 1;
            ...
            if (listener->listener_slot >= 0)
            {
               ...
               //     do_send = true;
				else if ((listener->user_platform.compare("scanner") == 0 || listener->user_platform.compare("3dscanner") == 0 || listener->user_platform.compare("stabilise") == 0 || listener->user_platform.compare("odmr")==0) && listener->send_counts_requested && (counter > sentlines))
					do_send = true;
            ...
            if (do_send)
            {
				...
				if ((listener->user_platform.compare("scanner") == 0 || listener->user_platform.compare("3dscanner") == 0 || listener->user_platform.compare("stabilise") == 0 || listener->user_platform.compare("odmr"))
					&& px_resolution_y > sentlines && listener->histogram_counts.size() > 0)
...
//printf("sending: '%s'\n", out.c_str());
                ...
                // Clear the values for next time
					...
					if (listener->user_platform.compare("stabilise") == 0 || listener->user_platform.compare("scanner") == 0 || listener->user_platform.compare("3dscanner") == 0 || listener->user_platform.compare("odmr")==0) {
						...
						if (sentlines > (px_resolution_y - 1)) {
							startflag = 0;
							odmr_startflag = 0;
