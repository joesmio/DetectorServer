...
//printf("]]] lockok %d:%s\n", __LINE__, __FILE__);fflush(stdout);
    ...
    for (size_t index = 0; index < current_listeners.size(); ++index)
    {
		...
		else
		{
			...
			if (histogram_channels < 400 && histogram_channels > 10)
				px_resolution = histogram_channels;

			listener->histogram_counts.resize(px_resolution);

			for (uint32_t index = 0; index < px_resolution; index++) {
				listener->histogram_counts[index].resize(px_resolution
				);
				for (uint32_t i = 0; i < px_resolution; i++)
            ...
            if (count)
            {
                ...
                for (int index = 0; index < count; index++)
                {
					...
					if (startflag == 2 && chan[index] == 3) {

						// Channel 3 indicates the pulse train has gone high. This means stop counting photons! Find the duration and get ready to move to the next line in the histogram
						startflag = 1;

						// Calculate the duration of this count
						duration = time[index] - start;

						if (counter < px_resolution)
	...
	bool scannerconnected = 0;
		for (size_t index = 0; index < current_listeners.size(); ++index)
        {
            ...
            if (do_send)
            {
				...
				if (size(listener->histogram_counts) > sentlines)
...
{
    printf("Sarting Waterloo access thread...\n");
...
void capture_loop()
{
    ...
    if (timetag)
    {
        ...
        write_to_restart_log("Conneced to Waterloo box ok.");
