...
//printf("]]] lockok %d:%s\n", __LINE__, __FILE__);fflush(stdout);
    ...
    for (size_t index = 0; index < current_listeners.size(); ++index)
    {
		...
		//printf("user_platform: '%s'\n", listener->user_platform.c_str());
		...
		else
		{
			...
			//Do this if user platform is g2

			if (listener->user_platform.compare("g2") == 0){

				// In this first instance we will just return g2 of two defined channels but could be extended to get g2 of multiple channels, FL, etc
				...
				//printf("user g2 detected \n");

				listener->histogram_counts.resize(1);
				listener->histogram_counts[0].resize(6400);

				//listener->histogram_counts[0].resize(400);

				// Resolution is ~ 156 ps (1/64 ns)
				// We want 1600*(1/64)


				// Normal resolution is 1/2 ns and we want bins up to 800 ns so 800 * 2 bins
				

				//Change to bins of 2 ns i.e. time  4
				...
				if (count) {
					...
					for (int index = 0; index < count; ++index)
					{
						...
						if (chan[index] == 1) {
							//start
							startflag_g2 = 1;
							listener->timestamp = time[index];
						}

						if (chan[index] == 2 && startflag_g2 == 1) {
							//stop
							listener->incr = (time[index] - listener->timestamp) / 16; // (uint32_t)(nsec_to_timetag_units(listener->coincidence_windows_ns[0]));
							listener->px = (listener->px + listener->incr) % listener->histogram_channels; //try modulo again...
							listener->histogram_counts[0][listener->px]++;
							startflag_g2 = 0;
						}
					}
			...
			else if (listener->user_platform.compare("fast") == 0) {

				// here the user decides when to get counts, when to start histo, when to pause, when to stop
				// crucially: user also decides binwidth of histo so we need time information

				// start --> by virtue of being connected
				// send histogram --> whenever user wants it
				// stop --> on user disconnect
				...
				if (count) {
					...
					if (listener->startflag == 0) {
						...
						listener->histogram_counts.resize(1);
						listener->px_resolution_y = 1; // eventually can be one row for each channel of interest
						listener->px_resolution_x = listener->histogram_channels;

						listener->histogram_counts[0].resize(listener->histogram_channels);
						listener->incr = 0;

						// clear out values on start
						for (uint32_t i = 0; i < listener->histogram_channels; i++)
							listener->histogram_counts[0][i] = 0;
					}

					for (int index = 0; index < count; index++)
					{	

						if ((listener->startflag == 0) && (chan[index]==7)) {
							listener->timestamp = time[index];
							listener->px = 0;
							listener->startflag = 1;
							listener->counter = 0;
						}
						/*
						...
						if (listener->startflag == 1) {
							...
							//printf("%i",time[index]);
							//printf(" compared to");
							//printf("%i",listener->timestamp);
							//printf("\n");

							if (time[index] > ((uint32_t)listener->timestamp + (uint32_t)nsec_to_timetag_units(listener->coincidence_windows_ns[0])) )
							{
								//time to move on to next bin

								//which bin?
								listener->incr = (time[index] -listener->timestamp) / (uint32_t)nsec_to_timetag_units(listener->coincidence_windows_ns[0]);
								...
								//printf("PING");
								//printf(" timebin %i \n", (uint32_t)nsec_to_timetag_units(listener->coincidence_windows_ns[0]));
							}

							if ((listener->px) > (listener->histogram_channels - 1))
								break;

							if ((chan[index] == 1) || (chan[index] == 2)) {
								// add up counts from first two count channels, update histogram
								listener->histogram_counts[0][listener->px]++;
						...
						*/
						if (listener->startflag == 1) {
							...
							if ((chan[index] == 1) || (chan[index] == 2)) {
								...
								listener->incr = (time[index] - listener->timestamp) / 64; // (uint32_t)(nsec_to_timetag_units(listener->coincidence_windows_ns[0]));

								// 1 increment is 1/6.4 ns ~ 156 ps
								// set resolution of 8 ns --> 52 bins is 8.125 ns 
								// better... 64 bins is 10 ns
						...
						if ((chan[index] == 7)) {
							...
							listener->timestamp = time[index];
							listener->px = 0;
							listener->counter++; // How many sweeps have been performed?
						}
				...
				else if (listener->user_platform.compare("fast-gated") == 0) {

					// here we use the channel 7 on the waterloobox as a gate, from channel 3 on spincore

					if (count) {

						if (listener->startflag == 0) {
							listener->timestamp = time[0];
							listener->startflag = 1;
						...
						for (int index = 0; index < count; index++)
						{
							...
							if (listener->startflag == 1) {
								...
								if (chan[index] == 7)
								{
									//time to move on to next bin, modulo how many channels there are
									(listener->px) = (listener->px + 1) % listener->histogram_channels;
								}
			...
			else if (listener->user_platform.compare("odmr") == 0)
			{
				...
				if (listener->startflag == 0) {
					...
					listener->histogram_counts.resize(1);
					listener->px_resolution_y = 1;
					listener->px_resolution_x = histogram_channels;

					listener->histogram_counts[0].resize(histogram_channels);

					for (uint32_t i = 0; i < histogram_channels; i++)
						listener->histogram_counts[0][i] = 0;
				}
				...
				if (count)
				{
					...
					for (int index = 0; index < count; index++)
					{
						...
						// Uncomment to check biasing conditions met
						/*
						if (chan[index] == 15)
						printf("HIGH ");
						...
						*/
						...
						if (listener->startflag == 0) {
							// Start scan because we have received the initial rising pulse
							//printf("start scan \n");
							listener->startflag = 1;
							listener->counter = 0;
							listener->px = 0;
							listener->sentlines = 0;

						}

						if (listener->startflag == 1 && chan[index] == 13) {
							// Start counting photons, falling edge means source stable
							listener->startflag = 2;

							//duration = time[index] - start;
							//printf("PING");
							//printf(" %i \n", duration / 6400);

							listener->start = time[index];
							listener->countme = 0;

							//printf("Start counting photons \n");

						}


						if (listener->startflag == 2 && chan[index] == 14) {

							// Channel 14 rising when to stop i.e. microwave unstable

							// Calculate the duration of this count
							duration = time[index] - listener->start;

							//start = time[index];

							if (duration > 6400 * 1000) {
								// prevent glitch if duration ns ( or should this be less than 1 ms

								listener->startflag = 1;

								if (listener->counter < listener->px_resolution_y) {

									//if (listener->user_platform.compare("stabilise") == 0)
									//listener->histogram_counts[counter][px] = countme;

									//else
									listener->histogram_counts[listener->counter][listener->px] = (int)((6400000000 / duration)*listener->countme);
									
									
									/*printf("ODMR ");
									printf("@ %i ", listener->counter);
									printf("PONG %i ", listener->px);
									printf("counts %i ", listener->histogram_counts[listener->counter][listener->px]);
									printf(" duration %i \n", duration/ 6400);*/
								}

								// New line if at end of line in either direction
								if (listener->px == histogram_channels - 1) {

									listener->counter++;
									listener->px = 0;

									//printf("] \n");
									//printf("On line %i, ", listener->counter);
									//printf("sent %i \n", listener->sentlines);
									//printf("[");
						...
						if (((chan[index] == 1) || (chan[index] == 2)) && (listener->startflag == 2)) {
							listener->countme++;
						}
					}
				}
				...
				if ((listener->startflag == 0)  || (listener->histogram_counts.size() == 0)){
					...
					if (listener->user_platform.compare("scanner") == 0) {
						...
						if (histogram_channels < 400 && histogram_channels > 5) {
							listener->px_resolution_x = histogram_channels;
							listener->px_resolution_y = histogram_channels;

						}
					}

					if (listener->user_platform.compare("3dscanner") == 0) {

						uint32_t z_res = listener->slide_co_width; // proxy without change to listener class

						if (histogram_channels < 400 && histogram_channels > 5) {
							listener->px_resolution_x = histogram_channels;
							listener->px_resolution_y = z_res*histogram_channels;

						}
					}


					if (listener->user_platform.compare("stabilise") == 0) { // tracking

						if (histogram_channels < 400 && histogram_channels > 1) {
							listener->px_resolution_x = histogram_channels;
							listener->px_resolution_y = 3; // x, y, z  ? * number of tries i.e. 3 * 65

						}
					}

					listener->histogram_counts.resize(listener->px_resolution_y);

					for (uint32_t index = 0; index < listener->px_resolution_y; index++) {
						listener->histogram_counts[index].resize(listener->px_resolution_x
						);
						
						for (uint32_t i = 0; i < listener->px_resolution_x; i++)
							listener->histogram_counts[index][i] = 0;
				...
				if (count)
				{
					...
					for (int index = 0; index < count; index++)
					{
						...
						*/
						...
						if (listener->startflag == 1 && chan[index] == 15) {
							// Start counting photons, gone high
							listener->startflag = 2;
					...
					//Variables should be listener specific
					//	listener->


						if (listener->startflag == 2 && chan[index] == 16) {

							// Channel 16 indicates the pulse train has gone low. This means stop counting photons! Find the duration and get ready to move to the next line in the histogram
							...
							if (duration > 6400 * 1000) {
								...
								if (listener->counter < listener->px_resolution_y) {
									...
									listener->histogram_counts[listener->counter][listener->px] = (int)((6400000000 / duration)*listener->countme);
									//printf("PONG ");
									//printf("res %i", listener->px_resolution_x);
									//printf("@ %i ", listener->counter);
									if (listener->px > (listener->px_resolution_x-2))
										printf("PONG %i ", listener->px);
									//printf("counts %i ", listener->histogram_counts[listener->counter][listener->px]);
									//printf(" %i \n", duration/ 6400);
								}
								...
								// New line if at end of line in either direction
								if (listener->px == (listener->px_resolution_x - 1)) {

									listener->counter++;
									listener->px = 0;

									//printf("] \n");
									//printf("On line %i, sent %i \n", listener->counter, listener->sentlines);
									//printf("sent %i \n", sentlines);
									//printf("[");
    ...
    // Send the output to this listener
    {
		...
		for (size_t index = 0; index < current_listeners.size(); ++index)
        {
            ...
            if (listener->listener_slot >= 0)
            {
				...
				//     do_send = true;
				else if ((listener->user_platform.compare("scanner") == 0 || listener->user_platform.compare("3dscanner") == 0 || listener->user_platform.compare("stabilise") == 0 || listener->user_platform.compare("odmr") == 0) && listener->send_counts_requested && (listener->counter > listener->sentlines))
				{
					do_send = true;
					//printf("do send \n");
				}
				//else if ((listener->user_platform.compare("stabilise") == 0) && (listener->send_counts_requested) && (counter > 2) ) { // taking away listener counts requested as never flags
					//do_send = true; // send every 3: x, y, z
					//startflag = 0; 
				//}
				else if ((listener->user_platform.compare("fast") == 0) && (listener->send_counts_requested)) {
					do_send = true;
					listener->send_counts_requested = false;
					//for (uint32_t i = 0; i < listener->histogram_channels; i++)
						//listener->histogram_counts[0][i] = 0;
					
					//listener->startflag = 0;
            ...
            if (do_send)
            {
				...
				if ((listener->user_platform.compare("scanner") == 0 || listener->user_platform.compare("3dscanner") == 0 || listener->user_platform.compare("stabilise") == 0 || listener->user_platform.compare("odmr"))
					&& listener->px_resolution_y > listener->sentlines && listener->histogram_counts.size() > 0)
                out.append("histogram_counts", listener->histogram_counts[listener->sentlines]);
				else out.append("histogram_counts", listener->histogram_counts);
                out.append("counts", listener->single_counts);
				out.append("counter",listener->counter);
                out.terminate();
...
//printf("sending: '%s'\n", out.c_str());
                ...
                // Clear the values for next time
					...
					if (listener->user_platform.compare("stabilise") == 0 || listener->user_platform.compare("scanner") == 0 || listener->user_platform.compare("3dscanner") == 0 || listener->user_platform.compare("odmr")==0) {
						listener->sentlines++;

						if (listener->sentlines > (listener->px_resolution_y - 1)) {
							listener->startflag = 0;
							listener->clear_all_counts();
							listener->sentlines = 0;
							listener->counter = 0;
							printf("Sent everything to %s, ready for new stuff \n", listener->user_platform);
