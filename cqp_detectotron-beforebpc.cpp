...
//printf("]]] lockok %d:%s\n", __LINE__, __FILE__);fflush(stdout);
    ...
    for (size_t index = 0; index < current_listeners.size(); ++index)
    {
		...
		else
		{
			...
			if (listener->user_platform.compare("g2") == 0){
				...
				listener->histogram_counts[0].resize(1600);
				...
				if (count) {
					...
					for (int index = 0; index < count; ++index)
					{
						...
						if (chan[index] == 2 && startflag == 1) {
							//stop
							t = time[index] - start; 
							if (t < 51200) // 64 * 800 ns
							listener->histogram_counts[0][t/32 ]++; // get t in terms of 0.5 ns i.e. /32
	...
	bool scannerconnected = 0;
		for (size_t index = 0; index < current_listeners.size(); ++index)
        {
			...
			if (listener->user_platform.compare("scanner") == 0 || listener->user_platform.compare("stabilise") == 0 || listener->user_platform.compare("3dscanner") == 0)
				scannerconnected = 1;
            ...
            if (listener->listener_slot >= 0)
            {
               ...
               //     do_send = true;
				else if ((listener->user_platform.compare("scanner") == 0 || listener->user_platform.compare("3dscanner") == 0) && listener->send_counts_requested && (counter > sentlines))
					do_send = true;
            ...
            if (do_send)
            {
				...
				if ((listener->user_platform.compare("scanner") == 0 || listener->user_platform.compare("3dscanner") == 0) && px_resolution_y > sentlines && listener->histogram_counts.size() > 0)
...
//printf("sending: '%s'\n", out.c_str());
                ...
                // Clear the values for next time
					...
					if (listener->user_platform.compare("scanner") == 0 || listener->user_platform.compare("3dscanner") == 0) {
						sentlines++;
